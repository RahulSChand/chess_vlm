...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse text sections and save to pickle file')
    parser.add_argument('--input', type=str, required=True, help='Input text file to process')
    parser.add_argument('--output', type=str, required=True, help='Output pickle file to save results')
    
    args = parser.parse_args()
    
    print(f"Parsing text sections from {args.input}...")
    results = process_file(args.input)
    
    if results:
        print(f"Processed {len(results)} sections")
        print(len(results))
        save_pickle(results, args.output)
