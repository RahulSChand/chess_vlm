...
class ChessUI(QWidget):
    ...
    def place_piece(self, sqr_name, piece):
        ...
        if isinstance(piece, str):
            # This handles calls from reset_board() or direct calls with FEN symbols
            piece_symbol_str = piece
        elif hasattr(piece, 'symbol') and callable(getattr(piece, 'symbol')):

            # This handles chess.Piece objects from refresh_from_state (if position.piece_at returns them)
            piece_symbol_str = piece.symbol()


        if isinstance(piece_symbol_str, str):
            piece_symbol = piece_symbol_str
        else:
            piece_symbol = PIECE_CONVERSION.get(piece)


        # print(f"piece_symbol: {piece_symbol}, {type(piece_symbol)}")
        if not piece_symbol:
            print(f'Unknown piece v1: {piece}')
            return


        # print(f"piece_symbol: {piece_symbol}")
        # print(f"repr(piece_symbol): {repr(piece_symbol)}") # This is the key addition
        # print(f"PIECE_MAP contains key? {piece_symbol in PIECE_MAP}")
        # print(f"PIECE_MAP: {PIECE_MAP}")
        piece_image = PIECE_MAP.get(piece_symbol)
        # print(f"piece_image: {piece_image}")
        if not piece_image:
            print(f'Unknown piece v2: {piece_symbol}')
            return

        piece_label = PieceLabel(self, piece_image)
        pixmap_path = f'./assets/pieces/{piece_image}.png'  # noqa
